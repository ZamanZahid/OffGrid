...
/**
 * Reads the fused gravity vector (and optionally the rotation-vector attitude) around
 * the moment of capture. Average over a short still window for a clean "down" vector --
 * this is the single most accuracy-critical input (1 deg tilt ~ 110 km).
 *
 * Usage: start() just before/at the shutter, hold the phone still ~1-2 s, then read
 * averagedGravity().
 */
class SkySensors(context: Context) : SensorEventListener {
    ...
    @Volatile var latestQuat: FloatArray? = null
        private set
    ...
    /** Mean gravity over the captured window, as a DoubleArray [x, y, z]. */
